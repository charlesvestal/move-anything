...
$ssh_root "for name in MoveOriginal Move shadow_ui move-anything audio_stream_daemon; do pids=\$(pidof \$name 2>/dev/null || true); if [ -n \"\$pids\" ]; then kill -9 \$pids 2>/dev/null || true; fi; done"
$ssh_ableton "sleep 1"
