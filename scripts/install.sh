...
$ssh_root "for name in MoveMessageDisplay MoveLauncher Move MoveOriginal move-anything shadow_ui audio_stream_daemon; do pids=\$(pidof \$name 2>/dev/null || true); if [ -n \"\$pids\" ]; then kill \$pids || true; fi; done"
$ssh_root "sleep 0.5"
$ssh_root "for name in MoveMessageDisplay MoveLauncher Move MoveOriginal move-anything shadow_ui audio_stream_daemon; do pids=\$(pidof \$name 2>/dev/null || true); if [ -n \"\$pids\" ]; then kill -9 \$pids || true; fi; done"
$ssh_root "sleep 0.2"
...
# Deploy audio stream daemon if present
if $ssh_ableton "test -d /data/UserData/move-anything/usb_audio"; then
    echo "Audio stream daemon deployed to /data/UserData/move-anything/usb_audio/"
    $ssh_ableton "chmod +x /data/UserData/move-anything/usb_audio/audio_stream_daemon 2>/dev/null || true"
    $ssh_ableton "chmod +x /data/UserData/move-anything/usb_audio/stop_daemon.sh 2>/dev/null || true"
fi
...
$ssh_root "for name in MoveOriginal Move MoveLauncher MoveMessageDisplay shadow_ui move-anything audio_stream_daemon; do pids=\$(pidof \$name 2>/dev/null || true); if [ -n \"\$pids\" ]; then kill -9 \$pids 2>/dev/null || true; fi; done"
