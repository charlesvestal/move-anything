...
/* usb_midi_bridge_relativize_set(enabled) - Write to shared memory + persist to features.json */
static JSValue js_usb_midi_bridge_relativize_set(JSContext *ctx, JSValueConst this_val,
                                                  int argc, JSValueConst *argv) {
    (void)this_val;
    if (argc < 1 || !shadow_control) return JS_UNDEFINED;
    ...
    int enabled = 0;
    JS_ToInt32(ctx, &enabled, argv[0]);
    shadow_control->usb_midi_bridge_relativize = enabled ? 1 : 0;
    ...
    /* Check if key already exists */
    char *key = strstr(buf, "\"usb_midi_bridge_relativize\"");
    if (key) {
        /* Replace the value */
        char *colon = strchr(key, ':');
        if (colon) {
...
/* usb_midi_bridge_relativize_get() -> bool - Read from shared memory */
static JSValue js_usb_midi_bridge_relativize_get(JSContext *ctx, JSValueConst this_val,
                                                  int argc, JSValueConst *argv) {
    (void)this_val; (void)argc; (void)argv;
    if (!shadow_control) return JS_NewBool(ctx, 0);
    return JS_NewBool(ctx, shadow_control->usb_midi_bridge_relativize != 0);
}
...
static void init_javascript(JSRuntime **prt, JSContext **pctx) {
    ...
    JS_SetPropertyStr(ctx, global_obj, "usb_midi_bridge_relativize_set", JS_NewCFunction(ctx, js_usb_midi_bridge_relativize_set, "usb_midi_bridge_relativize_set", 1));
    JS_SetPropertyStr(ctx, global_obj, "usb_midi_bridge_relativize_get", JS_NewCFunction(ctx, js_usb_midi_bridge_relativize_get, "usb_midi_bridge_relativize_get", 0));
